...
//  populates the elements of matrix m with arbitrary double numbers.
...
//prints the values of the matrix in tabular format.
void print(const Matrix& m)
{
...
//prints the matrix in tabular format, plus sum of its rows and columns
void print_with_sum(const Matrix& m)
{
    if(m.get_rows() == 0 && m.get_cols() == 0){
...
//returns true if any charactres in str is not a digit
bool not_all_digits(const char* str){
